#!/usr/bin/env python3
"""
Jellyfin NFO Generator - Interactive Edition
从 Bangumi API 抓取元数据并生成 Jellyfin 兼容的 NFO 文件。
使用 bangumi_ids.csv 作为元数据缓存，避免重复抓取。

用法:
  python3 generate_nfo.py              # 全量模式：处理所有条目
...
CACHE_PATH = '/server/Media/Anime/bangumi_cache.json'
SCRIPT_PATH = '/server/Media/Anime/generate_nfo.py'
HEADERS = {'User-Agent': 'Mozilla/5.0 (compatible; NFOGenerator/2.0; +https://github.com/bob/jellyfin-nfo)'}
CACHE_TTL = 86400 * 7  # 缓存有效期 7 天
API_DELAY = 0.5        # API 请求间隔（秒）

# ======== 统计 ========
stats = {'series': 0, 'movies': 0, 'episodes': 0, 'skipped': 0, 'errors': 0, 'cache_hits': 0, 'api_calls': 0}
...
# ======== NFO 生成 ========
def write_xml(path, root_tag, fields):
...
def process_series(row, cache, force_refresh):
    """处理一个 TV 系列"""
    ...
    if not os.path.exists(tvshow_path) or force_refresh:
        write_xml(tvshow_path, 'tvshow', tvshow_fields)
    ...
    for snum, sdir in enumerate(season_dirs, 1):
        ...
        for vf in video_files:
            ...
            for k, v in [('title', ep_title), ('originaltitle', ep_orig), ('showtitle', title),
                         ('season', str(snum)), ('episode', str(ep_num)),
                         ('plot', ep_plot), ('aired', ep_aired), ('runtime', ep_dur)]:
                if v:
                    lines.append(f'  <{k}>{esc(v)}</{k}>')
            eid = str(ep_data.get('id', ''))
            if eid:
                lines.append(f'  <uniqueid type="bangumi" default="true">{esc(eid)}</uniqueid>')
...
def process_movie(row, cache, force_refresh):
    """处理一部电影"""
    name = row['series_name']
    folder = row['actual_folder']
    bid = row['bangumi_id']
    ...
    write_xml(nfo_path, 'movie', movie_fields)
    stats['movies'] += 1
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Jellyfin NFO Generator from Bangumi')
    ...
    # Banner
    print("=" * 60)
    print("  Jellyfin NFO Generator v2.0")
    ...
    # 读取 CSV（新格式：cn_name,jp_name,year,folder_name,bangumi_id,status）
    ...
    with open(CSV_PATH, 'r', encoding='utf-8-sig') as f:
        ...
        for row in reader:
            ...
            if bid and bid.isdigit():
                entries.append({
                    'series_name': row.get('cn_name', '').strip(),
    ...
    movies = [r for r in matched if not r['_is_tv']]
    
    log(f"待处理: TV {len(tv)} 部, 电影 {len(movies)} 部", 'INFO')
    print("-" * 60)
    ...
    for i, row in enumerate(tv):
        ...
        process_series(row, cache, args.refresh)
    
    # 处理电影
    for i, row in enumerate(movies):
        print()
        log(f"[{i+1}/{len(movies)}] 处理电影: {row['series_name']}", 'PROG')
        process_movie(row, cache, args.refresh)
